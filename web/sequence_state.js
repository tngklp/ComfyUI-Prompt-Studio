...
Chunk Direction governs this interval. For refinement, apply the requested change to the current prompt while considering the accepted neighbors. Return only the full final prompt, without commentary, planning or JSON.`;
...
const PREVIOUS_SEQUENCE_INSTRUCTIONS = `Write only the requested target clip as a complete prompt in the supplied official guide's format. Keep all required sections. Sequence metadata is planning context, never output prose: target time starts at zero and ends at the local duration. Translate an absolute event time by subtracting this clip's global start; events outside its interval belong elsewhere. Shot numbering restarts at [Shot 1] in every target clip, with no timestamp on its first shot.
...
The current Chunk Direction governs this interval. For refinement apply the requested change to the current prompt, considering the supplied neighbors. Return only the complete final prompt, without commentary, planning, JSON or instructions to another writer.`;

// Migrate only the untouched prototype default; preserve every user-authored instruction.
export const LEGACY_SEQUENCE_INSTRUCTIONS = `Write consecutive parts of one video sequence. Chunk boundaries are technical generation boundaries, not shot cuts. Read the whole Creative Brief and outline, including absolute time ranges, but write only the requested chunk as a complete, self-contained prompt using the official guide supplied with the request.
