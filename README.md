...
| Target | Category | Modes |
| --- | --- | --- |
| MiniMax H3 | Video | T2VA, I2VA, FL2VA, L2VA, Reference |
| MiniMax Music 3 | Audio | Caption, Lyrics |
| Qwen Image 2.1 | Image | Text to Image, Image Edit |

Targets are declared as data in [`targets.json`](targets.json) and resolved by [`backend/targets/`](backend/targets/), so supporting another model means adding a descriptor plus a strategy module rather than editing core logic. The generation target is separate from the *prompt model* — the local or remote LLM that actually writes the prompt.

## Highlights

- Multiple generation targets: video, music, and image prompts from one workspace, with per-target modes, limits, and output contracts.
...
![Prompt Studio](docs/assets/v0.3/reference-workspace.png)
...
You do not need to write MiniMax section headings, timestamps, or reference syntax by hand. Describe the video and tell Prompt Studio what each reference should contribute:
